...
# *         Errores:  Javacasm                         * #
# *                                                    * #
# *     Experimento Bio v1.35     Fecha: 13/11/2019     * #
# ****************************************************** #
##########################################################
*/


#define __VERSION__ "Experimento Biotecnoencuentro 3 v1.35"

#define USE_BME280 1
#define USE_LCDI2C 1
#define USE_18x20  1
#define USE_CONDUCTIVIDAD 1
...
int LUZ_VERDE = -100;       //inicializado fuera de rango
float TEMPERATURA_LIQUIDO = -100; //inicializado fuera de rango
int CONDUCTIVIDAD = -100;   //inicializado fuera de rango
...
{ 
  ...
  TEMPERATURA_LIQUIDO = sensors.getTempC(insideThermometer);
#endif

#ifdef USE_CONDUCTIVIDAD
  CONDUCTIVIDAD = 1023-analogRead(PIN_Sensor_Humedad);
#endif
  ...
  
{
  ...
  if (Serial.available() > 0) {
    ...
    if (lectura == '*' OR lectura == '+'){  // indicador para mandar una muestra unica al puerto serie
     ...
     if (lectura == 'h' OR lectura == 'c'){  // indicador para mostrar que es cada dato y el orden
      mostarCabecerasEnPython();
      return 0;  //ok, peticion de cabeceras
      }      
...
void mostarCabecerasEnPython()
{ 
 Serial.flush();
  
  /* FORMATEO de datos y ENVIO al puerto SERIE */
  Serial.print(F("#")); // Para indicar que no son datos
  ...
  Serial.print(F("PPM"));
  Serial.print(F("**"));
  Serial.print(F("LUZ_AZUL"));
  Serial.print(F("**"));
  Serial.print(F("LUZ_ROJA"));
  Serial.print(F("**"));
  Serial.print(F("PRESION"));
  Serial.print(F("**"));
  Serial.print(F("CONDUCTIVIDAD"));
  Serial.print(F("**"));
  Serial.print(F("TEMPERATURA_LIQUIDO"));  
  Serial.print(F("**"));
  Serial.println(F("LUZ_VERDE")); 
}
...
{ 
  ...
  Serial.print(F("**"));
  Serial.print(TEMPERATURA_LIQUIDO);  
  Serial.print(F("**"));
  Serial.println(LUZ_VERDE); 
}
