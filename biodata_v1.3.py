...
################   " <---------- METADATOS -------------->   <------- SENSORES -------> "
cabeceraTXTdatos = "n muestra\tESTACION\tFECHA\tHORA\tTemperatura\tHumedad\tPH\tCO2\tAzul\tRojo\tPresion"
...
ultimo_valor_sensor_7_valido = 0
                ...
                try:
                    ...
                    if comando == "/info":
                        send_message ("============================\n" +
                                      ...
                                      " Rojo:   " + str(valor_sensor_5_Now) + " \n" +
                                      " Azul:   " + str(valor_sensor_6_Now) + " \n" +
                                      " Presion:   " + str(valor_sensor_7_Now) + " atm\n"                                       
                                      , chat_id)
...
def dibujar_grafica(datos_brutos_experimento):
    ...
    try:
        ...
        addGrafico(711,'red',0,'Temp',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
        addGrafico(712,'green',1,'Humedad',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
        addGrafico(713,'pink',6,'Presion',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
        addGrafico(714,'blue',2,'pH',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
        addGrafico(715,'black',3,'CO2',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
        addGrafico(716,'yellow',4,'Red Led',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
        addGrafico(717,'orange',5,'Blue Led',puntos_eje_x, datos_por_sensor_y,etiqueta,orden,minimos_maximos)
            ...
            if time.time() >= minuto_adquisicion_datos + TIEMPO_ENTRE_MUESTRAS:
                ...
                if muestra_datos != None and str(type(muestra_datos)) == "<class 'list'>":
                    ...
                    if muestra_datos[0] !=None and muestra_datos[1] !=None and muestra_datos[2] !=None:
                        ...
                        if len(muestra_datos) > 6:
                            valor_sensor_7_Now = muestra_datos[6] # PRESION
                            ultimo_valor_sensor_7_valido = muestra_datos[6]
                ...
                #de los usuarios que se produzcan mientras se obtienen datos correctos
                    ...
                    if FLAG_reinicio_Arduino == False: #esta bandera es para evitar que se traten de conservar
                        ...
                        valor_sensor_7_Now = ultimo_valor_sensor_7_valido
        except:
